...
保留真人原声，处理重拍和气口，添加分层字幕，交付 MP4、SRT、可编辑工程及质检结果。
```
...
制作时可以说“只要 MP4”“导出剪映可编辑工程”或“两者都要”。Agent 在 `production.json` 记录选择，并使用同一时间轴生成交付。

剪映工程包含独立视频切段及完整原素材、可编辑文字字幕、独立 BGM/配音轨，方便手动调整气口、音乐和文字。可选脚本依赖与用法见 [剪映工程导出](skills/video-production/references/jianying-export.md)。支持新草稿生成、素材打包和搬运后的路径重定位；原 Remotion 复杂动效、重点短语样式和字体未自动还原。
