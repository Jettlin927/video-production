...
选择剪映工程交付时按 [剪映工程导出](../../video-production/references/jianying-export.md) 把同 revision 的 edit-plan、字幕和附加轨道写入草稿。复制完整源素材以保留可恢复的气口，不把所有片段提前拼成单段文件。剪映内手动改动后，原计划和旧 MP4 不再代表当前工程；复查切口、字幕和 BGM 联动，重新记录版本及验收结果。只有工程、没有 MP4 时，编码相关检查标为不适用，工程实际打开和编辑保存检查不能省略或由 JSON 校验替代。
